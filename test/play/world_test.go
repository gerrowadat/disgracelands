...
// against reference/tools/skilloracle.c and tests a function. This tests
// the thing somebody types, which is the distinction this suite exists for
// — and the two halves of find_skill_num's behaviour that reached players
// are both here: the per-word abbreviation (#355) and the empty name
// (#365).
func TestSpellNamesAsAPlayerTypesThem(t *testing.T) {
	...
	// More words than the name has is not a match: the C's verdict is
	// `ok && !*first2`, and the query has words left over.
	contains(t, "a query longer than the spell name",
		c.do("cast 'magic missile extra'"), "Cast what?!?")

	// An unterminated quote is not an error — strtok's second call runs to
	// the end of the line when it finds no closing delimiter (#358).
	if unterminated := c.do("cast 'magic missile"); unterminated != full {
		t.Errorf("cast with no closing quote answered differently:\n got: %s\nwant: %s",
			unterminated, full)
	}

	// And the one nobody would guess. Only the quote is a strtok
	// delimiter, so `cast '  '` hands two spaces to find_skill_num,
	// any_one_arg tokenises them away, and the word loop never runs —
	// leaving `ok` true and the query exhausted, which matches the *first*
	// spell in the table. Armor, at level one, for free.
