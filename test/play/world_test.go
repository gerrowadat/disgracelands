...
// TestAbbreviatedSpellNamesReachTheSpell is `cast 'mag mis'`, which is what
// a caster actually types and which this server refused until #355.
//
// find_skill_num has two matching rules and the port had one: the whole
// typed string against the whole spell name, but not the second pass that
// walks both a word at a time. So `magic mis` worked and `mag mis` did not,
// along with 1,144 other abbreviations of the game's own spell names
// (docs/investigations/partial-matching.md §4).
//
// internal/game has the exhaustive comparison against
// reference/tools/skilloracle.c. This is here because that one tests a
// function and this tests the thing a player types, through a real socket,
// on a server that read its spell table off disk — which is the distinction
// this suite exists for.
func TestAbbreviatedSpellNamesReachTheSpell(t *testing.T) {
	m := start(t, mini)
	c := m.dial()
	...
	// abbreviation of it. Asserted as equality rather than against a fixed
	...
	for _, abbrev := range []string{
		"cast 'magic mis'", // rule 1: the whole string as a prefix
		"cast 'mag mis'",   // rule 2: the first word abbreviated -- the bug
		"cast 'm mis'",     // one letter of it
		"cast 'mag missile'",
	...
	// The other half: more words than the name has is *not* a match, because
	// the C's verdict is `ok && !*first2` and the query has words left over.
