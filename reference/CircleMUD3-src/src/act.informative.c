...
void look_at_room(struct char_data *ch, int ignore_brief)
{
  ...
  if ((!IS_NPC(ch) && !PRF_FLAGGED(ch, PRF_BRIEF)) || ignore_brief ||
    ...
    send_to_char(world[IN_ROOM(ch)].description, ch);
  ...
  if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_GOOD_REGEN))
  {
	  ...
	  send_to_char("You feel a soft, warm feeling in your bones.", ch);
	  send_to_char(CCNRM(ch, C_NRM), ch);
  	  send_to_char("\r\n", ch);
  }
  if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_PKILL))
  {
	  ...
	  send_to_char("You have entered a ", ch);
	  send_to_char(CCNRM(ch, C_NRM), ch);
	  send_to_char(CCRED(ch, C_NRM), ch);
	  send_to_char("[Player Killer]", ch);
	  send_to_char(CCNRM(ch, C_NRM), ch);
	  send_to_char(CCYEL(ch, C_NRM), ch);
	  send_to_char(" room. Beware!", ch);
	  send_to_char(CCNRM(ch, C_NRM), ch);
  	  send_to_char("\r\n", ch);
  }
  /* </DoC> */
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    if (short_list) {
      sprintf(buf, "%s[%2d %s] %-12.12s%s%s",
	      (GET_LEVEL(tch) >= LVL_IMMORT ? BMAG : ""),
	      GET_LEVEL(tch), CLASS_ABBR(tch), GET_NAME(tch),
	      (GET_LEVEL(tch) >= LVL_IMMORT ? KNRM : ""),
	      ((!(++num_can_see % 4)) ? "\r\n" : ""));
      send_to_char(buf, ch);
    } else {
      num_can_see++;
	  ...
	  /* Only do this for actual remorts! */
	  if (prevclasses > 0)
	  {
	  	prevclasses --;
	  }
...
/* <DoC> */
          ...
          if (GET_CLASS(tch) == CLASS_PALADIN)
          {
                        if (IS_SET(PSF_FLAGS(tch),PSF_UNWORTHY))
                strcat(buf," (UNWORTHY)");

                        if (IS_SET(PSF_FLAGS(tch),PSF_FALLEN))
                strcat(buf," (FALLEN)");
          }
