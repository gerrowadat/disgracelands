...
void gain_exp(struct char_data *ch, int gain)
{
  ...
  if (gain > 0) {
	...
	/*
	 * <DoC> - Check level-based gain limit
	 * Cannot gain more then current level exp over 10.
	 */
	current_lvl_xp = level_exp(GET_CLASS(ch), GET_LEVEL(ch) + 1) - level_exp(GET_CLASS(ch), GET_LEVEL(ch));
	if (gain > (current_lvl_xp/10))
	{
		gain = (current_lvl_xp/10);
		send_to_char("You can only understand so much...\n\r",ch);
	}
	/* </DoC> */
    GET_EXP(ch) += gain;
    ...
    if (is_altered) {
      ...
      if (num_levels == 1)
        ...
        send_to_char(buf, ch);
        /* <DoC> */
        snprintf(buf,sizeof(buf),"A voice whispers in your ear, '%s has gained %d levels!!!'\r\n", GET_NAME(ch),num_levels);
        send_to_all_color(buf,KCYN);
        /* </DoC> */
      }
      set_title(ch, NULL);
  ...
  if (!IS_NPC(ch)) {
    ...
    if (is_altered) {
      ...
      if (num_levels == 1)
        ...
        send_to_char(buf, ch);
        /* <DoC> */
        snprintf(buf,sizeof(buf),"A voice whispers in your ear, '%s has gained %d levels!!!'", GET_NAME(ch),num_levels);
        send_to_all_color(buf, KCYN);
        /* </DoC> */
      }
      set_title(ch, NULL);
