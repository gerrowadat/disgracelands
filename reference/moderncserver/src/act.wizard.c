...
ACMD(do_remort)
{
        ...
        else if (!*buf2)
        {
                ...
                if (!(victim = get_char_vis(ch, buf, NULL, FIND_CHAR_WORLD)))
                {
                        send_to_char("That is not a valid player name, or they are not logged in.\r\n", ch);
                }
                else
                {
                        old_vector = GET_REMORT_VECTOR(victim);
                        send_to_char("Player currently has access to skills/spells of:",ch);
                        for (i=0; i< NUM_CLASSES; i++)
                        {
                                if (((int)pc_class_remort_masks[i] & old_vector) == (int)pc_class_remort_masks[i])
                                {
                                        send_to_char(" ", ch);
                                        send_to_char(pc_class_snames[i],ch);
                                }
                        }
                        send_to_char(".\r\n", ch);
                }
        ...
        else
        {
                ...
                else
                {
                        ...
                        else
                        {
                                ...
                                {
                                        snprintf(buf2,sizeof(buf2),"But %s is already a %s! To undo a remort, try 'remort %s -%s'. Remember, you cannot undo a remort if the player is currently that class.\r\n",GET_NAME(victim),(char *)pc_class_snames[i],GET_NAME(victim),(char *)pc_class_snames[i]);
                                        send_to_char(buf2,ch);
                                }
                                else
                                {
                                        ...
                                        if (undo == 0)
                                        {
                                                snprintf(buf2,sizeof(buf2),"%s remorted to become a %s!\r\n",GET_NAME(victim),pc_class_snames[i]);
                                        }
send_to_char(buf2,ch);
                                        send_to_char("This player now has access to skills/spells of:",ch);

                                        if (undo == 0)
                                        {
                                                snprintf(buf2,sizeof(buf2),"You fall to the ground, clutching your chest, as an unearthly force bestows new knowledge and powers on you!\r\nYou gain the skills and privileges of a %s!\r\n",pc_class_snames[i]);
                                        }
                                        else
                                        {
                                                snprintf(buf2,sizeof(buf2),"You sink to the ground, aghast, as you feel your %shood slip away!\r\n",pc_class_snames[i]);
                                        }
                                        send_to_char(buf2,victim);

                                        if (undo == 0)
                                        {
                                                snprintf(buf2, sizeof(buf2), "A voice whispers in your ear, 'All hail %s! Living again as a %s!\r\n",GET_NAME(victim),pc_class_snames[i]);
                                                send_to_all_color(buf2,KCYN);
                                        }
...
void do_stat_object(struct char_data *ch, struct obj_data *j)
{
  ...
  strcpy(buf2, GET_OBJ_SPEC(j) ? "Exists" : "None");
  ...
  sprintbit(GET_OBJ_AFFECT(j), affected_bits, buf);
  strcat(buf, "\r\n");
  send_to_char(buf, ch);
...
ACMD(do_wizutil)
{
  ...
  if (!*arg)
	...
	/* <DoC> */
        case SCMD_REDEEM:
          if (!IS_SET(PSF_FLAGS(vict),PSF_FALLEN))
          {
                  send_to_char("Your victim has not fallen!\r\n",ch);
                  return;
          }
          REMOVE_BIT(PSF_FLAGS(vict),PSF_FALLEN);
          send_to_char("Redeemed.\r\n",ch);
          send_to_char("You feel your paladinly powers restored! Rejoice! You live again in God's glory!\r\n",vict);
          snprintf(buf,sizeof(buf),"(GC) %s redeemed by %s", GET_NAME(vict), GET_NAME(ch));
          mudlog(buf, BRF, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE);
          break;	
	/* </DoC> */
...
ACMD(do_show)
{
  ...
  case 2:			/* player */
    ...
    sprintf(buf + strlen(buf),
	    ...
	    birth, ctime(&vbuf.last_logon), vbuf.played / 3600,
	    vbuf.played / 60 % 60);
    send_to_char(buf, ch);
    break;
  case 3:
		...
		char *val_arg)
{
  ...
  case 45:
    ...
    *(GET_PASSWD(vict) + MAX_PWD_LENGTH) = '\0';
	sprintf(buf, "(GC) %s has set password for %s.", GET_NAME(ch), GET_NAME(vict));
	mudlog(buf, BRF, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE);
