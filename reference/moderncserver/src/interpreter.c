...
cpp_extern const struct command_info cmd_info[] = {
  ...
  { "reload"   , POS_DEAD    , do_reboot   , LVL_IMPL, 0 },
   ...
   /* <DoC> */
  { "redeem"   , POS_DEAD    , do_wizutil  , LVL_GRGOD, SCMD_REDEEM },
  { "remort"   , POS_DEAD    , do_remort   , LVL_IMPL, 0 },
  /* </DoC> */
  ...
  { "wizlock"  , POS_DEAD    , do_wizlock  , LVL_GRGOD, 0 },
...
void nanny(struct descriptor_data *d, char *arg)
{
  ...
  case CON_NAME_CNFRM:		/* wait for conf. of new name    */
    if (UPPER(*arg) == 'Y') {
		/* <DoC> Allow new players from SELECT-banned sites </DoC> */
  ...
  case CON_QCLASS:
    ...
    sprintf(buf, "%s [%s] new player.", GET_NAME(d->character), d->host);
        ...
        /* For a new player, set their remort vector to their class */
        d->character->player_specials->saved.remort_vector = pc_class_remort_masks[(int)GET_CLASS(d->character)];
		/* Also, set their color to be complete */
		SET_BIT(PRF_FLAGS(d->character), (PRF_COLOR_1 * (3 & 1)) | (PRF_COLOR_2 * (3 & 2) >> 1));
		/* And prompt all... */
		SET_BIT(PRF_FLAGS(d->character), PRF_DISPHP | PRF_DISPMANA | PRF_DISPMOVE);
		/* *sigh* And autoexit.... */
		TOGGLE_BIT(PRF_FLAGS(d->character), (PRF_AUTOEXIT));
		/* Sometimes siteok is off for new players */
		SET_BIT(PLR_FLAGS(d->character), PLR_SITEOK);
        /* </DoC> */
