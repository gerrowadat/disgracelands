...
/* <DoC> */
extern int pc_class_remort_masks[];
/* </DoC> */
...
/* <DoC> */
int get_magicuser_class(struct char_data *ch);
/* </DoC> */
...
ACMD(do_cast)
{
  ...
  struct obj_data *tobj = NULL;
  char *s, *t;
  int mana, spellnum, i, j, spell_lvl_ok, target = 0;
  ...
  /* 
   * No casting for silenced folk.
   */
  if (AFF_FLAGGED(ch, AFF_SILENCE))
  {
	  send_to_char("You try, but the words simply fail you.\r\n",ch);
	  return;
  }

  /* <DoC>
   * Fallen/unworthy Paladin stuff.
   */

  if (GET_CLASS(ch) == CLASS_PALADIN)
  {
        /* G'way, naughty sinners */
        if (IS_SET(PSF_FLAGS(ch), PSF_FALLEN))
        {
                send_to_char("You exchanged that spell for a sinful existence, scum!\r\n",ch);
                return;
        }
        if ((IS_SET(PSF_FLAGS(ch), PSF_UNWORTHY))&&(GET_ALIGNMENT(ch) <= 600)&&(GET_ALIGNMENT(ch) > -350))
        {
                send_to_char("You must repent your sins before you may use this spell again!\r\n",ch);
                return;
        }
        if (GET_ALIGNMENT(ch) < 0)
        {
                  if ((GET_ALIGNMENT(ch) < -350)&&(!IS_SET(PSF_FLAGS(ch),PSF_FALLEN)))
                  {
                          /* Paladin is Fallen */
                          send_to_char("Alas! Your evil has been your downfall! You may never again bear the holy name of Paladin! Begone, sinner!\r\n",ch);
                          snprintf(buf, sizeof(buf),"A voice whispers in your ear, 'Rejoice! The sinner %s has been cast out!'",GET_NAME(ch));
                          send_to_all_color(buf,KCYN);
                          SET_BIT(PSF_FLAGS(ch),PSF_FALLEN);
                          REMOVE_BIT(PSF_FLAGS(ch),PSF_UNWORTHY);
                          return;
                  }
                else if ((GET_ALIGNMENT(ch) > -350)&&(!IS_SET(PSF_FLAGS(ch),PSF_UNWORTHY)))
                  {
                          /* Paladin is Unworthy */
                          send_to_char("You are unworthy of using that spell. Repent your sins, lest you be judged by the boot of God himself!\r\n",ch);
                          SET_BIT(PSF_FLAGS(ch), PSF_UNWORTHY);
                          return;
                  }
        }
        ...
        {
                  /* If they are unworthy, worthy them */
                  if (IS_SET(PSF_FLAGS(ch),PSF_UNWORTHY))
                  {
                          send_to_char("Welcome back, friend! May your sword and its might bear great witness to almighty God!",ch);
                          REMOVE_BIT(PSF_FLAGS(ch), PSF_UNWORTHY);
                  }
        }
  }
...
/* </DoC>  */
  ...
  /* <DoC>
   * Replacing the below commented stmnt, to test all the classes of the user.
   *
  ...
  */
  j = GET_REMORT_VECTOR(ch);
  for (i = 0; i < NUM_CLASSES; i++)
  {
          if ((j & (int)pc_class_remort_masks[i]) == (int)pc_class_remort_masks[i])
          {
                  /* The player is this class, check the spell level. */
...
/* <DoC> */
  spello(SPELL_DISPEL_MAGIC, "dispel magic", 100, 70, 3, POS_STANDING,
...
/* </DoC> */
  ...
  spello(SPELL_OUCHIE, "ouchie", 40, 30, 2, POS_FIGHTING,
        TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_DAMAGE,
        NULL);
	
  spello(SPELL_IMMOLATE, "immolate", 40, 30, 2, POS_FIGHTING,
        TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_DAMAGE,
        NULL);
  ...
  spello(SPELL_HOLY_SHIELD, "holy shield", 80, 40, 2, POS_STANDING,
        TAR_CHAR_ROOM | TAR_SELF_ONLY, FALSE, MAG_AFFECTS,
        "You feel your holy protection fade.");

  spello(SPELL_HOLY_SMITE, "holy smite", 80, 40, 2, POS_STANDING,
                  TAR_CHAR_ROOM | TAR_SELF_ONLY, FALSE, MAG_AFFECTS,
                  "You feel less smitey.");
  ...
  spello(SPELL_SILENCE, "silence", 100, 70, 3, POS_FIGHTING,
	TAR_CHAR_ROOM, FALSE, MAG_AFFECTS,
	"Outside noises return to you once more.");
