...
ACMD(do_assist)
{
  ...
  if (!*arg)
    ...
    else if ((GET_MOB_VNUM(opponent) == 14205) && (GET_LEVEL(ch) < LVL_IMMORT))/*Yoss - medusa*/
    {
...
ACMD(do_hit)
{
  ...
  if (!*arg)
    ...
    if ((GET_POS(ch) == POS_STANDING) && (vict != FIGHTING(ch))) {
      ...
      if ((GET_MOB_VNUM(vict) == 14205) && (GET_LEVEL(ch) < LVL_IMMORT))/*Yoss - medusa*/
      {
        send_to_char("Medusa turns her eyes to you!\r\nYou flee head over heals.\r\n", ch);
        char_from_room(ch);
        char_to_room(ch, real_room(14284));
...
ACMD(do_flee)
{
  ...
  for (i = 0; i < 6; i++) {
	...
	if (was_fighting && !IS_NPC(ch)) {
	  ...
	  /* <DoC> */
	  /* No XP spooge for PKILLs. */
	  if (IS_NPC(was_fighting)||IS_NPC(ch))
	  {
	  	gain_exp(ch, -loss);
	  }
	  /* </DoC> */
...
ACMD(do_rescue)
{
  ...
  if ((GET_MOB_VNUM(tmp_ch) == 14205) && (GET_LEVEL(ch) < LVL_IMMORT))/*Yoss - medusa*/
  {
    send_to_char("Medusa turns her eyes to you!\r\nYou flee head over heals.\r\n", ch);
    char_from_room(ch);
    char_to_room(ch, real_room(14284));
