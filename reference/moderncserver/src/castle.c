...
SPECIAL(king_welmar)
{
  ...
  case 'o':
    do_gen_door(ch, strcpy(actbuf, "door"), 0, SCMD_UNLOCK);
    do_gen_door(ch, strcpy(actbuf, "door"), 0, SCMD_OPEN);
    break;

  case 'c':
    do_gen_door(ch, strcpy(actbuf, "door"), 0, SCMD_CLOSE);
    do_gen_door(ch, strcpy(actbuf, "door"), 0, SCMD_LOCK);
    break;
...
SPECIAL(DicknDavid)
{
  ...
  return (block_way(ch, cmd, arg, castle_virtual(36), 1));
}
