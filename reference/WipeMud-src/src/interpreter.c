...
cpp_extern const struct command_info cmd_info[] = {
  ...
  { "file"     , POS_SLEEPING, do_file     , LVL_GRGOD, 0 },
...
void nanny(struct descriptor_data *d, char *arg)
{
  ...
  case CON_QCLASS:
	...
	/* <DoC> */
	send_to_all_color(KCYN,"A voice whispers in your ear, 'All hail %s, a newcomer!'\r\n",GET_NAME(d->character));

	/* For a new player, set their remort vector to correct value */
	d->character->player_specials->saved.remort_vector = (int)pc_class_remort_masks[(int)GET_CLASS(d->character)];
	/* We gotta save the char, too. */
	save_char(d->character,GET_LOADROOM(d->character));
