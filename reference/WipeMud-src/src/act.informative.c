...
void look_at_room(struct char_data *ch, int ignore_brief)
{
  ...
  /* <DoC> */
  if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_GOOD_REGEN))
  {
      send_to_char(ch,CCBLU(ch, C_NRM));
      send_to_char(ch,"You feel a soft, warm feeling in your bones.");
  ...
  if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_PKILL))
  {
      ...
      send_to_char(ch,"You have entered a ", ch);
      send_to_char(ch,CCNRM(ch, C_NRM));
      send_to_char(ch,CCRED(ch, C_NRM));
      send_to_char(ch,"[Player Killer]");
      send_to_char(ch,CCNRM(ch, C_NRM));
      send_to_char(ch,CCYEL(ch, C_NRM));
      send_to_char(ch," room. Beware!");
      send_to_char(ch,CCNRM(ch, C_NRM));
      send_to_char(ch,"\r\n");
  }
  /* </DoC> */
...
#define WHO_FORMAT \
"format: who [minlev[-maxlev]] [-n name] [-c classlist] [-d racelist] [-s] [-o]
[-q] [-r] [-z]\r\n"
...
ACMD(do_who)
{
  ...
  /* <DoC> */
  int temp_remort_vector,prevclasses,vector_index;
  /* </DoC> */
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    if (showclass && !(showclass & (1 << GET_CLASS(tch))))
     ...
     if (showrace && !(showrace & (1 << GET_RACE(tch))))
      ...
      /* <DoC>
	   ** Work Out colours based on number of classes.
	   ** Normal peons are in white.
	   ** first remorters in Dark green
	   ** second in bright green
	   ** third in dark yellow
	   ** fourth in bright yellow
	   **/
		temp_remort_vector = GET_REMORT_VECTOR(tch);
		prevclasses = 0;
	    for(vector_index = (NUM_CLASSES-1); vector_index >= 0; vector_index--)
	    {
	         if (temp_remort_vector >= pc_class_remort_masks[vector_index])
	         {
	              prevclasses++;
	              temp_remort_vector -= pc_class_remort_masks[vector_index];
	         }
	    }

      /* Don't count current class, which will be int he remort vector */
      prevclasses --;
	  ...
	  {
		  case -1:
			  send_to_char(ch,"%s",BBLU);
			  break;

		  case 1:
			  send_to_char(ch,"%s",KGRN);
			  break;

		  case 2:
			  send_to_char(ch,"%s",BGRN);
			  break;

		  case 3:
			  send_to_char(ch,"%s",KYEL);
			  break;

		  case 4:
			  send_to_char(ch,"%s",BYEL);
			  break;
    ...
    if (short_list) {
      send_to_char(ch, "[%2d %s %s] %-12.12s%s",
	      GET_LEVEL(tch), CLASS_ABBR(tch), RACE_ABBR(tch), GET_NAME(tch),
	      ((!(++num_can_see % 4)) ? "\r\n" : ""));
    } else {
      num_can_see++;
      send_to_char(ch, "[%2d %s %s] %s %s",
	      GET_LEVEL(tch), CLASS_ABBR(tch), RACE_ABBR(tch), GET_NAME(tch),
	      GET_TITLE(tch));
