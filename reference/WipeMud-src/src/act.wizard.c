...
/* <DoC> */
extern const  int pc_class_remort_masks[];
/* </DoC> */
...
ACMD(do_show)
{
  ...
  case 11:
	i = 0;
	send_to_char(ch,"%sCurrently Available classes (starting classes in %s%sgreen%s):\r\n",KYEL,KNRM,KGRN,KNRM);
	for(i=0;i<NUM_CLASSES;i++)
	{
		/* If it's a starting class... */
		if (parse_class(pc_class_snames[i][0]) != CLASS_UNDEFINED)
		{
			send_to_char(ch,"%s%s%s ",KGRN,pc_class_snames[i],KNRM);
		}
		else
		{
			send_to_char(ch,"%s%s%s ",KYEL,pc_class_snames[i],KNRM);
		}
	}
	send_to_char(ch,"\r\n");
  break;
...
{
	struct char_data *victim;
	int i, old_vector, new_vector, mask, undo;
	char *newclass;
	char buf[MAX_INPUT_LENGTH],buf2[MAX_INPUT_LENGTH];
	...
	if (!*buf)
	{
		send_to_char(ch,"Whom do you wish to remort?\r\n");
	}
	else if (!*buf2)
	{
		if (!(victim = get_char_vis(ch, buf, NULL, FIND_CHAR_WORLD)))
		{
			send_to_char(ch,"'%s' is not a valid player name, or they are not logged in.\r\n",buf);
		}
		else
		{
			old_vector = GET_REMORT_VECTOR(victim);
			send_to_char(ch,"%s%s currently has access to skills/spells of:%s",KYEL,GET_NAME(victim),KNRM);
			/* send_to_char(ch,"DEBUG: Current remort vector : %d\r\n",old_vector); */
			for (i=0; i< NUM_CLASSES; i++)
			{
				if (((int)pc_class_remort_masks[i] & old_vector) == (int)pc_class_remort_masks[i])
				{
					send_to_char(ch, " %s",pc_class_snames[i]);
				}
			}
			send_to_char(ch,".\r\n");
		}
	...
	else
	{
		...
		else
		{
			...
			for(i=0; i < NUM_CLASSES; i++)
			{
				if (!strcasecmp((const char *)newclass,(const char *)pc_class_snames[i]))
				{
					break;
				}
			}
			...
			if (i == (NUM_CLASSES))
			{
				send_to_char(ch,"'%s' is not a valid class. See 'show classes' for a list.\r\n",newclass);
			}
			else
			{
				old_vector = GET_REMORT_VECTOR(victim);
				mask = (int)pc_class_remort_masks[i];
				/* Cannot remort to a current class, of course */
				if ((i == GET_CLASS(victim))||(((old_vector & mask) == mask) && (undo == 0)))
				{
					send_to_char(ch,"%sBut %s is already a %s! To undo a remort, try 'remort %s -%s'.\r\nRemember, you cannot undo a remort if the player is currently that class.\r\n%s",KYEL,GET_NAME(victim),(char *)pc_class_snames[i],GET_NAME(victim),(char *)pc_class_snames[i],KNRM);
				}
				else
				{
					/* Set the bit in the remort vector. Do this by ORing it with
					 * the appropriate vector mask in class.c
					 */
					 if (undo == 1)
					 {
					 ...
					 /* send_to_char(ch,"DEBUG: Setting remort_vector to %d for %s\r\n",new_vector,GET_NAME(victim)); */
					 victim->player_specials->saved.remort_vector = new_vector;
					 /* send_to_char(ch,"DEBUG: Remort vector is now: %d\r\n",GET_REMORT_VECTOR(victim)); */
					 save_char(victim,GET_LOADROOM(victim));

					 if (undo == 0)
					 {
						 send_to_char(ch,"%s%s remorted to become a %s%s%s!\r\n%s",KYEL,GET_NAME(victim),KCYN,pc_class_snames[i],KYEL,KNRM);
					 }

					 if (undo == 0)
					 {
						 send_to_char(victim,"You fall to the ground, clutching your chest, as an unearthly force bestows new knowledge and powers on you!\r\nYou gain the skills and privileges of a %s!\r\n",pc_class_snames[i]);
					 }
					 else
					 {
						 send_to_char(victim,"You sink to the ground, aghast, as you feel your %shood slip away!\r\n",pc_class_snames[i]);
					 }

					 if (undo == 0)
					 {
						 send_to_all("%sA voice whispers in your ear, 'All hail %s! Living again as a %s!\r\n%s",KCYN,GET_NAME(victim),pc_class_snames[i],KNRM);
					 }

					 send_to_char(ch,"%s%s now has access to the skills/spells of:%s",KYEL,GET_NAME(victim),KNRM);

					 for (i=0; i< NUM_CLASSES; i++)
					 {
						 if (((int)pc_class_remort_masks[i] & new_vector) == (int)pc_class_remort_masks[i])
						 {
							 send_to_char(ch,"%s %s%s",KCYN,pc_class_snames[i],KNRM);
						 }
					 }
					 send_to_char(ch,".\r\n");
