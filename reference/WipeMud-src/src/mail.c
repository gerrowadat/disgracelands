			  ...
			  int cmd, char *arg)
{
  ...
  //log ("%s writes a mail to %s",GET_NAME(ch), buf);
  //recipient = get_id_by_name(buf);
  //log ("%s got recipient # %d",GET_NAME(ch),recipient);
  //anus = mail_recip_ok(buf);
  //log ("mail_recip_ok is %d for recipient # %d\n",anus,recipient);
