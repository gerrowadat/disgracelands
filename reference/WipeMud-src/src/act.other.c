...
int count_items(struct char_data *ch, struct obj_data *i);
ACMD(do_group);
ACMD(do_ungroup);
ACMD(do_report);
ACMD(do_split);
...
/* <DoC> */
/* Pass in ch, and ch->carrying to obtain a count of items
 *  * Including contents of recurisve bag action.
 *   */
int count_items(struct char_data *ch, struct obj_data *i)
{
	struct obj_data *j;
	...
	for (j = i; j; j = j->next_content)
	{
		...
		{
			num_items += count_items(ch,j->contains);
		}
		num_items ++;
	}
...
ACMD(do_quit)
{
  ...
  /* <DoC> */
    ...
    /* Check Number of items, refuse to quit if number of items
	 *    * Exceeds Max Rent Bit.
	 **/

  num_items = count_items(ch,ch->carrying);
  for (poo = 0; poo < NUM_WEARS; poo ++)
  {
	  if (ch->equipment[poo] != NULL)
	  {
		  if (GET_OBJ_TYPE(ch->equipment[poo]) == ITEM_CONTAINER)
		  {
			  num_items += count_items(ch,ch->equipment[poo]->contains);
		  }
		  num_items ++;
	  }
  }

  if ((num_items > MAX_RENT)&&(GET_LEVEL(ch) < LVL_IMMORT))
  {
	  send_to_char(ch,"You currently have too many items (%d items in total).\r\nYou must have %d items or less before leaving.\r\n",
			  				num_items,MAX_RENT);
	  mudlog(NRM, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE, "%s attempted to quit with %d items.", GET_NAME(ch),num_items);
  }
  else
  {
	  send_to_char(ch,"Saving %d items.\r\n",num_items);
  }
...
int perform_group(struct char_data *ch, struct char_data *vict)
{
  ...
  /* <DoC> */
  /* Introducing Difference In level */

  /* Member is too low level */
  if (GET_LEVEL(ch) > GET_LEVEL(vict) + 7)
  {
      act("$N cannot hope to keep up with you!", FALSE, ch, 0, vict, TO_CHAR);
      act("You simply cannot keep up with $n's group.", FALSE, ch, 0, vict, TO_VICT);
      return(1);
  }

  if(GET_LEVEL(ch) < GET_LEVEL(vict) - 7)
  {
      act("$N is far too tough to group with little ol' you.", FALSE, ch, 0, vict, TO_CHAR);
      act("$n? C'mon, you can do better than that.", FALSE, ch, 0, vict, TO_VICT);
      return(1);
  }
  /* </DoC> */
