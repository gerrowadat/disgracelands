...
{
  struct char_data *mob;
  char mbitbuf[MAX_INPUT_LENGTH], abitbuf[MAX_INPUT_LENGTH];
 ...
 */
  ...
  case MEDIT_LEVEL:
    ...
    GET_MOVE(OLC_MOB(d))    = mob_lev*11;          /* hit point bonus         */
    GET_HIT(OLC_MOB(d))     = mob_lev/5;           /* number of hitpoint dice */
    GET_MANA(OLC_MOB(d))    = mob_lev/6;           /* size of hitpoint dice   */
    GET_HITROLL(OLC_MOB(d)) = mob_lev/8;           /* hitrol 0-12	      */
    GET_DAMROLL(OLC_MOB(d)) = mob_lev/2;           /* damroll 0-50	      */

      if (mob_lev <= 30)
        {
        GET_AC(OLC_MOB(d))   = (100-(mob_lev*4));  /* AC -20 to 96            */
        GET_NDD(OLC_MOB(d))  = MAX(1, mob_lev/20); /* number damage dice 1-5  */
        GET_SDD(OLC_MOB(d))  = MAX(2, mob_lev/16); /* size of damage dice 2-6 */
        GET_EXP(OLC_MOB(d))  = (mob_lev*mob_lev*100);
        GET_GOLD(OLC_MOB(d)) = (mob_lev*15);
        }
      else
        {
        GET_AC(OLC_MOB(d))   = 1-mob_lev;          /* AC -30 to -98	      */
	GET_NDD(OLC_MOB(d))  = MAX(5, mob_lev/12); /* number damage dice 5-8  */
	GET_SDD(OLC_MOB(d))  = MAX(6, mob_lev/12); /* size of damage dice 6-8 */
	GET_EXP(OLC_MOB(d))  = (mob_lev*mob_lev*30);
	GET_GOLD(OLC_MOB(d)) = (mob_lev*30);
	}
