...
void set_fighting(struct char_data *ch, struct char_data *vict)
{
  ...
  {
	  /* Fight! Fight! Fight! */
	  send_to_char(ch,"Okay! Let's get it on!\r\n");
	  send_to_char(vict,"Looks like %s wants a little...\r\n", GET_NAME(ch));
	  mudlog(BRF,LVL_IMMORT,TRUE, "%s started sanctioned pkill on %s at %s.",GET_NAME(ch),GET_NAME(vict), world[IN_ROOM(ch)].name);
  }
  /* </DoC> */
}
			     ...
			     struct char_data *victim)
{
  ...
  /* <DoC> */
  /* No XP or align change for pkills */
...
void solo_gain(struct char_data *ch, struct char_data *victim)
{
  ...
  if (exp > 1)
	...
	send_to_char(ch, "You recieve no experience! Hah!\r\n");
