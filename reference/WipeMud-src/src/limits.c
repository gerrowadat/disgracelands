...
void gain_exp(struct char_data *ch, int gain)
{
  ...
  if (gain > 0) {
	...
	if (gain > (current_lvl_xp/10))
	{
	     ...
	     send_to_char(ch,"You can only understand so much...\n\r");
	}
	/* </DoC> */
    GET_EXP(ch) += gain;
    ...
    if (is_altered) {
      ...
      if (num_levels == 1)
	  {
        send_to_char(ch, "You rise a level!\r\n");
	  	 send_to_all_color(KCYN,"A voice whispers in your ear, '%s has gained a level!'\r\n",GET_NAME(ch));
	  }
      else
	  {
			send_to_char(ch, "You rise %d levels!\r\n", num_levels);
			send_to_all_color(KCYN,"A voice whispers in your ear, '%s has gained %d levels!'\r\n",GET_NAME(ch),num_levels);
	  }
      set_title(ch, NULL);
