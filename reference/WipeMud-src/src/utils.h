...
/* <DoC> */
#define PSF_FLAGS(ch)   ((ch)->player_specials->saved.specflags)
#define GET_REMORT_VECTOR(ch)   CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.remort_vector))
/* </DoC> */
...
#define PERS(ch, vict)   (CAN_SEE(vict, ch) ? GET_NAME(ch) : GET_NAME(ch))
...
/*
 * <DoC>
 * These check for remort stuff now.
...
#define IS_MAGIC_USER(ch)   ((!IS_NPC(ch) && \
	                            (GET_CLASS(ch) == CLASS_MAGIC_USER)) || \
		                        ((!IS_NPC(ch)) &&(GET_REMORT_VECTOR(ch) & 1) == 1))
#define IS_CLERIC(ch)       ((!IS_NPC(ch) && \
	                            (GET_CLASS(ch) == CLASS_CLERIC)) || \
		                        ((!IS_NPC(ch)) &&(GET_REMORT_VECTOR(ch) & 2) == 2))
#define IS_THIEF(ch)        ((!IS_NPC(ch) && \
	                            (GET_CLASS(ch) == CLASS_THIEF)) || \
		                        ((!IS_NPC(ch)) &&(GET_REMORT_VECTOR(ch) & 4) == 4))
#define IS_WARRIOR(ch)      ((!IS_NPC(ch) && \
	                            (GET_CLASS(ch) == CLASS_WARRIOR)) || \
		                        ((!IS_NPC(ch)) &&(GET_REMORT_VECTOR(ch) & 8) == 8))
