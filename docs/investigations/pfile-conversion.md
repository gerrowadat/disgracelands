...
$ bin/pfiledump lib/pfiles/z/zod                   # freshly converted
lib/pfiles/z/zod                  name=Zod   sex=1 clas=3 lvl=34 birth=2006-02-10 last=2008-04-20 tags=247
```
...
`Reborn`'s actual `src/db.c` still uses the original binary
`load_char()`/`store_to_char()`/`char_to_store()` path at runtime — the
game doesn't read or write ascii pfiles when it's actually running. The
conversion above is a one-shot offline migration of the data, proven
readable, not yet a live-engine change.
...
- It's written against stock bpl17/19 `db.c`, which is very close to
  ...
  patch), but every call site that constructs/consumes a
  `struct char_file_u` needs updating too: `comm.c`'s login "nanny" state
  machine, `interpreter.c`, `act.wizard.c` (force/load-style god commands),
  `house.c`, `mail.c`, and the `src/util/` tools (`autowiz`, `mudpasswd`,
  `showplay`, `purgeplay`, `listrent`, `play2to3` all currently assume the
  binary format directly).
- `PLR_PREFIX`/`PLR_SUFFIX`/`SLASH` macros and the `lib/pfiles/<letter>/`
