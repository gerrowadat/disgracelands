...
| **2** | **A type per enumeration** — **under way** | §4.2. Class, Sex, Race, ItemType, Apply, SpellID, Sector, Liquid, and `MobDef.Position` onto the existing `game.Position`. `RemortVector` becomes `Set[Class]`. **Done: Class** (and the remort vector, which retired `Flags`), **Sex**, **ItemType**, **Apply**, **Sector**, **Race** (which turned out to enumerate nothing — see its doc comment). | Low, high volume. Watch the table-indexed tests (§5). |
