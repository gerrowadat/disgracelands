...
built so far has, and is not attempted yet either. So is the ordinary
weapon-swing damage message (§11 step 6c-ii) — `dam_message`'s compiled
table plus `skill_message`'s weapon-type half of `misc/messages`, still
`classic` only. Kick/bash/backstab/spell messages, game config, socials
and messages (the rest of §11 step 6c and §6/§7) are not; see §11 for
what landed and what is still a plan, and why those are staged separately
...
| **6c-ii. Damage messages (ordinary weapon swing) ✅, the rest not attempted** | `dam_message`'s compiled severity table and `skill_message`'s weapon-type half of `misc/messages`, ported for `internal/server/violence.go`'s `s.hit` only — the ordinary combat-round swing, replacing its two fixed strings with `damage()`'s real dispatch (a miss or death blow prefers a registered message; anything else always the compiled table). Kick, bash, backstab and every spell still print their own message (`Damage`'s doc comment), unaffected and unattempted; so is `config/messages.yaml` and game config (`config.c`'s tuning, currently scattered `const`s — a cross-cutting refactor independent of any file format). | Verified against the real archive (`data/misc/messages`, 55 records) as well as synthetic fixtures — a live server test proves a registered entry wins on a miss/death blow and loses to the compiled table on an ordinary hit, and that every other `Violence.Damage` caller stays silent, matching the scope this row describes. |
