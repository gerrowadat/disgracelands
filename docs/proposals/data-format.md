...
built so far has, and is not attempted yet either. So are real damage
messages (§11 step 6c-ii) — `dam_message`'s compiled table plus
`skill_message`'s full `misc/messages` lookup, covering the ordinary
weapon swing, kick, bash and backstab, still `classic` only. Every
spell's own message, `config/messages.yaml`, game config, socials and
messages (the rest of §11 step 6c and §6/§7) are not; see §11 for what
