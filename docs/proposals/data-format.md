...
bash, backstab and now every offensive spell too (§11 step 6c-v) — are
built, and on `native` as well as `classic` (§11 step 6c-iii,
...
Phase 5c. `config/help.yaml` and game config (the rest of §6/§7) are
not; see §11 for what landed and what is still a plan, and why those
