...
| **5f-ii. The rest of the economy** | Banking (`balance`, `deposit`, `withdraw`), `receive`, and the receptionist — `gen_receptionist`, `offer` and `rent` — which is the last piece of 5e and the same kind of specproc. | `spec_procs.c`, `objsave.c` |
