...
| Immortal odds and ends | `users` (:528), `skillset` (:469) and `reload` (:428). `wizhelp` (:553), `qecho` (:419) and `page` (:398) are built. `users` is the only one with any size to it — a hundred lines of flag parsing over the connection list. |
