...
| **5f. The economy** | Shops (`buy`, `sell`, `list`, `value`, `offer`), banking (`balance`, `deposit`, `withdraw`), `receive`, and the receptionist — `gen_receptionist`, `offer` and `rent` — which is the last piece of 5e and the same kind of specproc. Needs 5a. | `shop.c`, `objsave.c` |
