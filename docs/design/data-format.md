...
| **4. Flip the default** | `--world-format=yaml`, `data/` converted in the repo, `classic` demoted to import-only. | Not attempted, and now **planned in full**: `docs/proposals/yaml-only.md` is this row and row 7 together, taken past "flip the default" to "there is no other default". `--world-format=yaml` is available and works today; `classic` stays the default until that plan's row 4. |
...
| **7. Retire** | `ascii` and `binary` become `dlctl`-only; `classic` becomes import-only. | Not attempted, and now **planned in full**: `docs/proposals/yaml-only.md`, proposed as v1.0.0. Note one thing that plan sharpens about this row's own wording — retiring a format from the *server* is not deleting it from the *tree*. Every decoder here goes on being read, tested and released; `dlctl` simply becomes the only thing that reads them. §12's own "`classic` and `binary` are never deleted" paragraph below is unchanged by it. |
