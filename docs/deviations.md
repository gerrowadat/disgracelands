...
| **The prize, precisely** | Paladin is reached by remorting and by nothing else, and it takes having been **both a warrior and a cleric** — the rule the game ran on, enforced by `remort` since #262 (its own entry below). It is not a dead end: a paladin may remort onward, and the vector remembers the paladinhood. |
| **Where** | `internal/game/class.go`, asserted in `TestPaladinIsNotSelectableAtCreation`; the prerequisite in `TestPaladinTakesAWarriorAndACleric`. |
...
| | |
|---|---|
| **C** | `cast` alone gives *"Cast what where?"*; `cast   ` gives *"Spell names must be enclosed in the Holy Magic Symbols: '"*. The interpreter passes the argument with its spacing intact, so `strtok`'s first call has a token in one case and not in the other. |
...
| | |
|---|---|
| **C** | `do_remort` (act.wizard.c:355) sets one bit in the remort vector and stops. Class, level, experience, hit points, mana and skills are all left exactly as they were, so a level 30 warrior remorted to mage was a level 30 warrior who could also cast. |
| **Go** | The command does the rest, and *the rest is not invented*. Whoever ran the game typed a fixed sequence after every remort, and their notes for it are on issue #262: `set player class whatever` / `set player lessons 0` / `advance player 1` / `set player maxmana 100` / `set player maxmove 100` / *(hp's should be okay)* / `set player prime-stat-from-previous-class 18`. `game.Remort` is those lines, in that order, and the order is load-bearing — `advance 1` re-rolls the abilities, so pinning the old class's prime requisite has to come after it. |
| **Why** | Issue #262. The C's version leaves a character half-way through a class change with the other half in a wizard's muscle memory. A step that is always taken, always the same way, and disastrous to forget belongs in the command — and since the sequence was written down, the command can be exactly what was being done rather than somebody's reconstruction of it. |
| **The line worth understanding** | `set player prime-stat-from-previous-class 18` is the least obvious and the most important. `do_start` re-rolls for the *new* class, so the new prime requisite gets the best of six rolls and the old one gets what is left — while the remort vector means the character goes on being their old class too. Without that line they keep the old class's spells and lose the statistic that made them worth casting. Paladin's prime requisite is charisma, which is easy to get wrong: it is the one class whose roll ordering was never reachable at creation. `TestPrimeAbilityIsTheOneTheRollFavours` derives it from `RollAbilities` rather than restating it. |
| **Why 100 movement** | `init_char` gives 82. The notes say 100, twice, next to a mana figure that *is* init_char's. That is a choice somebody made rather than a slip, so it is reproduced and named (`remortMaxMove`) rather than quietly rounded to the stock value. |
| **What it does not do** | Gold, bank, equipment and played time are untouched — the notes do not mention them, and `do_start` does not either. Practices are zeroed and then given one level's worth back, which is what `lessons 0` followed by `advance 1` produces rather than a decision taken here. |
| **Two rules changed with it** | The C's second guard — refuse if the class's bit is already set — is **gone**. It meant "they already have those abilities", which was all remorting did; now that remorting is also a class change, having been a mage before is no reason to refuse to make somebody a mage now. The first guard stays: you cannot remort somebody into what they already are. And **paladin is refused unless the character has been both a warrior and a cleric** — the game's own rule, which the C never enforced (`do_remort` has no such check) and the wizards therefore kept by hand. "Has been" is the `IS_<CLASS>` question, so being a warrior right now counts and so does having been one and moved on. |
| **Paladin is remort-only, not a dead end** | You may remort *onward* from paladin to anything else, and the vector remembers: paladin's mask is 16 (`pc_class_remort_masks`, class.c:82) like every other class's. What paladin lacks is an `IS_PALADIN` macro reading that bit — utils.h:508 defines four, for mage, cleric, thief and warrior — which is a fact about how paladin powers are gated, not about whether having been one is recorded. Two comments in this port had that backwards and said paladin had no bit at all; both are corrected. |
| **Undo is untouched** | `remort <name> -<class>` is still a vector operation and nothing else. It does not un-set a class or hand back a level, because nothing records what those were before — the character has been played since. It does now refuse when the bit is not set, which makes the C's XOR (undoing a remort nobody had *grants* it, `docs/weirdnumbers.md`) unreachable through the command. |
| **Where** | `game.Remort` in `internal/game/class.go`, `doRemort` in `internal/session/remort.go`; `TestRemortFollowsTheHandRunProcedure` checks the sequence line by line, plus `TestRemortDoesTheHomework`, `TestPaladinTakesAWarriorAndACleric`, `TestAPaladinCanRemortOnward`, `TestRemortIntoAClassAlreadyHeld`, `TestUndoingARemortNobodyHadIsRefused`. |

### `do_start` does not wipe the skills array, and this port's did

| | |
|---|---|
| **C** | `do_start` (class.c:1802) has no `memset`. `SET_SKILL` (utils.h:325) writes one element of `player_specials->saved.skills`, and the six thief lines are the only writes in the function, so every skill already practised survives it. |
| **Go, until 2026-08-29** | `game.Start` did `rec.Skills = StartingSkills(rec.Class)` — a replacement. At creation the map is empty and the two are the same thing, which is why it went unnoticed. |
| **Where it bit** | `advance <name> 1`. Demotion runs `do_start` (`doAdvance`), so demoting a level 30 character cost them **every skill they had ever practised**, silently, on top of the level. Found while building #262's remort, which runs the same function. |
| **Now** | Merged, upwards only. |
| **Why it is here and not simply a bug** | It is a bug and it is fixed; the entry exists because "demotion also wipes your skills" is the kind of thing somebody could reasonably re-derive as intended behaviour from reading the Go, and this says it was not. |
