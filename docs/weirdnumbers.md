...
Verified against `reference/tools/skilloracle.c` (1,745 queries, including
the empty and whitespace ones its corpus lacked when it agreed with the C on
1,565 of 1,569) and `reference/tools/castparse.c` (do_cast's three strtok
calls, over every shape of quoting a player can type). The earlier
reasoning — that strtok skipping leading delimiters made an empty name
unreachable — is right about `cast ''` and wrong about `cast '  '`, and it
was reached by reading the function rather than running it.
