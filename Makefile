...
# reference/moderncserver/src/conf.h is generated by the C tree's own
# ./configure and is gitignored, so it is absent on a fresh checkout -- and
# without it every test that builds a C oracle against `struct char_file_u`
# skips: pfilegen.c and playerlayout.c both #include it.
#
# That is not the 32-bit skip, and conflating the two is what #244 cost.
# The gcc-multilib one takes the ilp32 half and leaves lp64 running;
# this one takes *both*, so `go test ./...` on a machine that has never
# configured the C tree comes back green having checked none of the
# pfile's layout at all -- for the one format that holds the real
# archived player records. #237 shipped a failing round-trip test that
# way, and #249 had to come back for it.
#
# So `make check` generates it. configure also substitutes CFLAGS into
# src/Makefile and src/util/Makefile, which are *tracked* and checked in
# carrying different values, so this saves and restores them: a target
# that dirties the worktree is a target nobody runs twice. Nothing else
# it writes is tracked.
#
# Best-effort on purpose: a machine with no C compiler cannot run these
# checks at all, and failing `make check` there would be trading a silent
# skip for a wall. It says what it could not do and carries on, which is
# the same posture `make parity` takes in scripts/release.sh.
CONF_H = reference/moderncserver/src/conf.h

.PHONY: c-conf
c-conf: ## Generate the C tree's conf.h, so the pfile layout checks run instead of skipping
	@if [ -f $(CONF_H) ]; then 	  echo "    $(CONF_H) is already there"; 	elif ! command -v gcc >/dev/null 2>&1 && ! command -v cc >/dev/null 2>&1; then 	  echo "==> no C compiler here, so $(CONF_H) cannot be generated;"; 	  echo "    the pfile layout and fixture checks will skip. release.yml runs them for real."; 	else 	  echo "==> configuring the C tree for $(CONF_H)"; 	  cp reference/moderncserver/src/Makefile /tmp/dl-c-src-makefile.$$$$ && 	  cp reference/moderncserver/src/util/Makefile /tmp/dl-c-util-makefile.$$$$ && 	  ( cd reference/moderncserver && CFLAGS="-std=gnu89 -fcommon -w" CC=gcc ./configure >/dev/null ) ; 	  status=$$?; 	  cp /tmp/dl-c-src-makefile.$$$$ reference/moderncserver/src/Makefile; 	  cp /tmp/dl-c-util-makefile.$$$$ reference/moderncserver/src/util/Makefile; 	  rm -f /tmp/dl-c-src-makefile.$$$$ /tmp/dl-c-util-makefile.$$$$; 	  if [ $$status -ne 0 ] || [ ! -f $(CONF_H) ]; then 	    echo "    ./configure did not produce $(CONF_H); the pfile layout checks will skip."; 	  fi; 	fi
...
check: c-conf ## Build, vet, format-check, lint, test, license and world-lint -- the full local check `make release` runs before tagging
	@unformatted=$$(gofmt -l .); \
