...
ACMD(do_quit)
{
  ...
  {
  	sprintf(buf,"You currently have too many items (%d items in total).\r\nYou must have %d items or less before leaving.\r\n",num_items,MAX_RENT);
  	send_to_char(buf,ch);
	sprintf(buf,"%s attempted to quit with %d items",GET_NAME(ch),num_items);
	mudlog(buf,NRM,MAX(LVL_IMMORT,GET_INVIS_LEV(ch)),TRUE);
	return;
  }
  else
  {
	  sprintf(buf,"Saving %d items.\r\n",num_items);
	  send_to_char(buf,ch);
  }
