...
{
	struct affected_type *curr, *next;

	if (victim == NULL)
		return;

	for(curr = victim->affected; curr; curr = next)
	{
		next = curr->next;
		affect_remove(victim,curr);
	}
}
/* </DoC> */
...
ASPELL(spell_summon)
{
  ...
  /* <DoC> */
  /* Only allow players to be summoned */
  if (IS_NPC(victim))
  {
	  send_to_char("Only players may be summoned.\r\n",ch);
	  return;
  }
  /* </DoC> */
...
/*humbug*/
    if (OBJ_FLAGGED(obj, ITEM_NO_LOCATE))
      continue;
/*humbug*/
...
ASPELL(spell_identify)
{
  ...
  if (obj) {
    ...
    if (GET_OBJ_AFFECT(obj)) {
      ...
      sprintbit(GET_OBJ_AFFECT(obj), affected_bits, buf);
      strcat(buf, "\r\n");
      send_to_char(buf, ch);
    }
