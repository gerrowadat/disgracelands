...
void set_fighting(struct char_data *ch, struct char_data *vict)
{
  ...
  {
	  /* Fight! Fight! Fight! */
	  send_to_char("Okay! Let's get it on!\r\n",ch);
	  sprintf(buf,"Looks like %s wants a little...\r\n",GET_NAME(ch));
	  send_to_char(buf,vict);
	  /* Only mudlog if both protagonists are players. */
	  if ((!IS_NPC(ch))&&(!IS_NPC(vict)))
	  {
	  	sprintf(buf,"%s started sanctioned pkill on %s at %s.",GET_NAME(ch),GET_NAME(vict), world[IN_ROOM(ch)].name);
	  	mudlog(buf,BRF,LVL_IMMORT,TRUE);
	  }
  }
...
{
  int share;
  int current_lvl_xp = level_exp(GET_CLASS(ch), GET_LEVEL(ch) + 1) - level_exp(GET_CLASS(ch), GET_LEVEL(ch));
  ...
  else
  {
	  if (share == 0)
	  {
    		send_to_char("You receive your share of experience -- Nothing! Ha!!\r\n", ch);
	  }
	  else
	  {
  ...
  /* <DoC> */
  /* No XP Gain for pkills. */
  if (IS_NPC(victim)||IS_NPC(ch))
  {
	  /* Also, mobs do not gain xp for killing lvl30 people */
	  /* (Or LVL_IMMORT-1 people, which is currently 30) */
	  if ((GET_LEVEL(victim) == LVL_IMMORT-1)&&(!IS_NPC(victim)))
	  {
		  if (!IS_NPC(ch))
		  {
			  gain_exp(ch,share);
...
void solo_gain(struct char_data *ch, struct char_data *victim)
{
  ...
  } else
  {
	  if (exp == 0)
	  {
    		send_to_char("You receive no experience! HA!.\r\n", ch);
	  }
	  else
	  {
		      ...
		      int w_type)
{
  ...
  static struct dam_weapon_type {
    ...
    {
      "$n PULVERISES $N with $s AMAZING #w!!",	/* 9: > 50   */
...
int damage(struct char_data *ch, struct char_data *victim, int dam, int attacktype)
{
  ...
  if (GET_POS(victim) == POS_DEAD) {
    ...
    if (!IS_NPC(victim)) {
		...
		if (!IS_NPC(ch))
		{
      		sprintf(buf2, "PKILL: %s killed by %s at %s", GET_NAME(victim), GET_NAME(ch),
   ...
   /* </DoC> */
	...
	/* <DoC> */
	/* <DoC> 
	 * Recover HP for mobs when they smite someone.
	 */
	if (IS_NPC(ch))
	{
		kill_recover = GET_MAX_HIT(ch)/20;

		if (GET_HIT(ch) < (GET_MAX_HIT(ch) - kill_recover))
		{
			GET_HIT(ch) += kill_recover;
		}
		else
		{
	...
	/* </DoC> */	
	/* Don't lose XP on pkill */
	if (!IS_NPC(ch) && !IS_NPC(victim))
	{
		/* PKILL */
		raw_kill(victim);
		sprintf(buf2,"A voice whispers in your ear, '%s has killed %s at %s!'\r\n",GET_NAME(ch), GET_NAME(victim),
					world[IN_ROOM(victim)].name);
		send_to_all_color(buf2,KCYN);
	}
	else
