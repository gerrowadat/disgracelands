...
ACMD(do_say)
{
  ...
  if (!*argument)
    send_to_char("Yes, but WHAT do you want to say?\r\n", ch);
	 ...
	 /* DoC - Drunknness stuff */
	  sprintf(buf, "$n says, '");
	  if (!IS_NPC(ch))
	  {
	  	if (GET_COND(ch,DRUNK) < 5)
	  	{
			strncat(buf, argument, strlen(argument)); 
			strncat(buf,"'",1);
	  	}
	  	else
	  	{
			  saying = malloc(256);
			  saying[0] = '\0';
	  		for (i=0;i<strlen(argument)&&strlen(saying)<240;i++)
	  		{
			  if (argument[i] == 's')
			  {
				  strncat(saying,"sh",2);
			  ...
			  {
				  strncat(saying,&argument[i],1);
			  }

    		}
			  if (number(0,2) == 0)
			  {
				  strncat(saying,"...*hic*.",9);
			  }
			  strncat(saying,"'",1);
		   	  strncat(buf, saying, strlen(saying));
	  	}
	}
	else
	{
