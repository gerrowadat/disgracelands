...
int new_descriptor(socket_t s)
{
  ...
  if (nameserver_is_slow || !(from = gethostbyaddr((char *) &peer.sin_addr,
    ...
    *(newd->host + HOST_LENGTH) = '\0';
	...
	/* <DoC> */
	sprintf(buf,"%s:%d", from->h_name, peer.sin_port);
    /* strncpy(newd->host, from->h_name, HOST_LENGTH); */
    strncpy(newd->host, buf, HOST_LENGTH); 
	/* </DoC> */
    *(newd->host + HOST_LENGTH) = '\0';
  }
...
/* <DoC> */
void send_to_all_color(const char *messg, const char *color)
{
        struct descriptor_data *i;
        ...
        for (i = descriptor_list; i; i = i->next)
        {
				/* Doesn't echo if a player is writing */
...
void brag(struct char_data *ch, struct char_data *vict)
{
	...
	char brag[256]={"       "};
	   ...
	   switch (number(0, 11)) {
		    case 0:
			   sprintf(brag, "$n brags, '%s was just too easy!'", GET_NAME(vict));
			break;
			case 1:
			   sprintf(brag, "$n brags, 'Time to go level some more, %s!'", GET_NAME(vict));
			break;
			case 2:
			   sprintf(brag, "$n brags, 'Bahaha! %s should stick to Fidos!'",GET_NAME(vict));
			break;
			case 3:
			   sprintf(brag, "$n brags, '%s is now in need of some exp...Muhaha!'", GET_NAME(vict));
	        break;
	   	    case 4:
		       sprintf(brag, "$n brags, '%s needs a hospital now.'",GET_NAME(vict));
			break;
			case 5:
			   sprintf(brag, "$n brags, 'What's wrong, %s? Can't get up?'", GET_NAME(vict));
			break;
			case 6:
			   sprintf(brag, "$n brags, 'EAT DIRT!'");
			break;
  			case 7:
			   sprintf(brag, "$n brags, '%s, your life force has just run out!'", GET_NAME(vict));
			break;
			case 8:
			   sprintf(brag, "$n brags, 'Bah, %s should stick to the newbie zone!'",GET_NAME(vict));
		    break;
			case 9:
			   sprintf(brag, "$n brags, '%s, give me your daughter's number and I might return your corpse. Aahaha!'", GET_NAME(vict));
			break;
			case 10:
			   sprintf(brag, "$n brags, 'Hey %s!  Come back, you dropped your corpse!'", GET_NAME(vict));
		    break;
			case 11:
			   sprintf(brag, "$n brags, 'I think %s wears pink chainmail.  Fights like a girl!'", GET_NAME(vict));
			break;
		}
