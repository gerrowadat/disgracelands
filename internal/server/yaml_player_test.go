...
// --player-format=yaml — where the deliberate deviation the user chose
// when scoping this (docs/design/data-format.md §8, player.StoredObject
// .Contains) is supposed to actually turn on. ascii/binary still come back
// loose (proven by the existing test, unmodified); this proves yaml does
// not.
func TestRentingUnderYamlKeepsTheRingInTheBag(t *testing.T) {
	store, err := yaml.New(player.Config{Dir: filepath.Join(t.TempDir(), "players")})
	if err != nil {
		t.Fatal(err)
	}
	...
	if loose != 1 {
		t.Errorf("carrying %d items loose, want 1 (the bag) -- yaml should not have flattened it", loose)
	}
	if inBag != 1 {
