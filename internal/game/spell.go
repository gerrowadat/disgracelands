...
// `cast 'mag mis'`.
//
// **A name with no words matches the first spell in the table**, which is
// armor, and that is the C's answer rather than a shrug. is_abbrev rejects
// an empty arg1, so rule 1 declines; the word loop then does not run at
// all, leaving `ok` TRUE and `first2` empty, so `ok && !*first2` holds on
// the very first entry.
//
// This was refused here until #365, and the reasoning it was refused on is
// worth keeping, because it was wrong twice in opposite directions. #355
// said the C could not reach an empty name, since do_cast takes it from
// `strtok(argument, "'")` and strtok skips leading delimiters. #358 then
// said it could, called the difference a deviation, and refused it on
// purpose. Both missed the same thing: **only the quote is a delimiter,
// and a space is not**.
//
...
//	cast '  '  -> find_skill_num("  ")  ->  armor     (the C, and here)
//
// find_skill_num cannot tell "  " from "" because any_one_arg tokenises
// the whitespace away before either rule looks at it. So two spaces
// between the quotes casts armor on the real server, at level one, for
// free — and does here.
//
// The refusal moved rather than being deleted. This function is
// find_skill_num and answers as it does; SpellNumberFromNameOrNumber is
// the *format* lookup and refuses an empty name itself, so a blank
// `spell:` in a yaml file stays an error instead of silently becoming
// armor.
//
// That split is deliberately the opposite of the call matchesKeywords
// makes for isname's own empty-string case, and the difference is
// reachability rather than taste: every isname caller checks for an empty
// argument first, and find_skill_num's callers do not all check —
// SPECIAL(guild) and do_skillset guard, do_cast does not.
...
// split is the point. This is what the yaml readers call — a player file's
// skills and affects, a wand's charge spell, a damage message's subject —
// where an empty name is a malformed file and should say so.
// SpellNumberByName is what a player's typing reaches, and there the C's
// own answer is armor (#365). A format that inherited that would turn a
// blank `spell:` into armor without a word about it.
