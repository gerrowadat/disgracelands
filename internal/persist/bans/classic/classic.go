...
// Package classic reads and writes the site ban list, porting ban.c.
//
...
func (s *Store) load() error {
	...
	for scanner.Scan() {
		...
		kind, ok := bans.ParseType(fields[0])
		if !ok {
		...
		ban := bans.Ban{Site: fields[1], Type: kind, By: fields[3]}
		if seconds != 0 {
...
// List implements bans.Store, newest first — the order the C's list is in,
// because it pushes each new one onto the front.
...
// Check implements bans.Store (isbanned, ban.c:82).
//
...
// Add implements bans.Store, reporting false when the site is already on
// the list — the C refuses rather than changing the type, and says so.
...
// Remove implements bans.Store (do_unban's half), reporting the ban that
// went.
