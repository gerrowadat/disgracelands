...
// classic holds, native holds too, and neither drops or truncates anything.
package native
...
// banSchema is the document's schema tag, docs/proposals/data-format.md
// §10.1.
...
func (s *Store) Add(ban bans.Ban) (bool, error) {
	...
	if len(ban.Site) > bans.MaxSiteLength {
		// native has no fixed-width field to enforce this, but truncating
		// silently would make the same site collide under classic and not
		// under native — kept for behavioural parity, not a format limit.
