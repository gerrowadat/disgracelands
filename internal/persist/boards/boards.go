...
// Package boards reads and writes bulletin board files, porting
...
// Message is one post as the file holds it.
type Message struct {
...
func New(dir string, readOnly bool) (*Store, error) {
	if dir == "" {
...
// Load reads one board's messages.
//
...
func (s *Store) Load(name string) ([]Message, error) {
...
// Save writes one board's messages, or removes the file when there are none —
// which is what Board_save_board does, and why an emptied board leaves no
// trace on disk.
func (s *Store) Save(name string, msgs []Message) error {
	if s.readOnly {
...
func decode(b []byte) ([]Message, error) {
	...
	for i := 0; i < count; i++ {
		...
		msgs = append(msgs, Message{Heading: heading, Level: level, Body: body})
	}
	return msgs, nil
}
