...
func (s *Source) WriteZone(_ context.Context, zone *game.ZoneDef, w *game.World) error {
	...
	if err != nil {
		return fmt.Errorf("native: zone %d: %w", zone.Vnum, err)
	}
...
func roomDocFrom(r *game.RoomDef) roomDoc {
	names, raw := game.NameBits(r.Flags, game.NativeRoomFlagNames())
	rd := roomDoc{
		Vnum:     int32(r.Vnum),
		Name:     Text(r.Name),
...
func sectorName(v int32) string {
	name, ok := game.NameByValue(v, game.NativeSectorNames())
	if !ok {
...
func doorName(flag int32) string {
	name, _ := game.NameByValue(flag, game.NativeExitDoorNames())
	return name
}

func mobDocFrom(m *game.MobDef) mobDoc {
	actFlags := m.ActionFlags.Clear(game.MobIsNPC) // never written; the reader force-sets it
	actNames, actRaw := game.NameBits(actFlags, game.NativeMobActFlagNames())
	affNames, affRaw := game.NameBits(m.AffectionFlags, game.NativeAffectFlagNames())
	...
	md := mobDoc{
		...
		Gold:            m.Gold,
		Exp:             m.Exp,
		Position:        valueName(m.Position, game.NativePositionNames()),
		DefaultPosition: valueName(m.DefaultPosition, game.NativePositionNames()),
		Sex:             valueName(m.Sex, game.NativeSexNames()),
	}
	if m.Enhanced {
...
func objDocFrom(o *game.ObjDef) objDoc {
	wearNames, wearRaw := game.NameBits(o.WearFlags, game.NativeWearFlagNames())
	flagNames, flagRaw := game.NameBits(o.ExtraFlags, game.NativeItemExtraFlagNames())
	permNames, _ := game.NameBits(game.Flags(o.PermAffect), game.NativeAffectFlagNames()) //nolint:gosec // affect bits fit comfortably
	...
	od := objDoc{
		...
		Type:       valueName(o.Type, game.NativeItemTypeNames()),
		Wear:       wearNames,
	...
	for _, a := range o.Affects {
		od.Affects = append(od.Affects, objAffectDoc{
			Location: valueName(a.Location, game.NativeApplyTypeNames()),
			Modifier: a.Modifier,
		})
	}
...
func shopDocFrom(sh *game.ShopDef) shopDoc {
	...
	for _, b := range sh.BuyTypes {
		sd.Buys = append(sd.Buys, shopBuyDoc{
			Type:    valueName(b.Type, game.NativeItemTypeNames()),
			Keyword: b.Keyword,
		})
	}
	...
	flagNames, _ := game.NameBits(sh.Flags, game.NativeShopFlagNames())
	sd.Flags = flagNames
	refuseNames, _ := game.NameBits(game.Flags(sh.TradeWith), game.NativeShopTradeNames()) //nolint:gosec // seven bits
...
func resetDocFrom(n ResetNode) resetDoc {
	...
	case game.ResetEquip:
		...
		d.Max = maxPtr(cmd.Arg2)
		d.Slot = valueName(cmd.Arg3, game.NativeWearPositionNames())
