...
// Typed object values, §4.3 of docs/proposals/data-format.md. Which of the
...
func TypedValues(objType int32, values [game.NumObjValues]int32) (typed any, unusedNonzero bool, ok bool) {
	...
	case game.ItemWeapon:
		...
		attackName, known := game.NameByValue(values[3], game.NativeAttackTypeNames())
		if !known {
	...
	case game.ItemDrinkCon:
		liquid, known := game.NameByValue(values[2], game.NativeLiquidNames())
		if !known {
			return nil, false, false
		}
		return DrinkValues{
...
func ValuesFromWeapon(v WeaponValues) (values [game.NumObjValues]int32, ok bool) {
	...
	attack, aok := game.ValueByName(v.DamageType, game.NativeAttackTypeNames())
	if !dok || !aok {
		return values, false
	}
...
func ValuesFromDrink(v DrinkValues) (values [game.NumObjValues]int32, ok bool) {
	liquid, lok := game.ValueByName(v.Liquid, game.NativeLiquidNames())
	if !lok {
		return values, false
	}
	poisoned := int32(0)
