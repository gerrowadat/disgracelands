...
func (l *loader) roomFromDoc(path string, rd roomDoc) *game.RoomDef {
	...
	sector, ok := game.ValueByName(rd.Sector, game.NativeSectorNames())
	if !ok {
		l.errorf("%s: room #%d: unknown sector %q", path, rd.Vnum, rd.Sector)
	}
	room.SectorType = sector
	room.Flags = l.flags(path, rd.Flags, game.NativeRoomFlagNames(), "room flag", rd.Vnum).Set(game.Flags(rd.FlagsRaw))
...
func (l *loader) mobFromDoc(path string, md mobDoc) *game.MobDef {
	...
	mob.ActionFlags = l.flags(path, md.Act, game.NativeMobActFlagNames(), "mob act flag", md.Vnum).Set(game.Flags(md.ActRaw)).Set(game.MobIsNPC)
	mob.AffectionFlags = l.flags(path, md.Affected, game.NativeAffectFlagNames(), "affect flag", md.Vnum).Set(game.Flags(md.AffectedRaw))
	...
	if pos, ok := game.ValueByName(md.Position, game.NativePositionNames()); ok {
		mob.Position = pos
	} else {
		l.errorf("%s: mob #%d: unknown position %q", path, md.Vnum, md.Position)
	}
	if pos, ok := game.ValueByName(md.DefaultPosition, game.NativePositionNames()); ok {
		mob.DefaultPosition = pos
	} else {
		l.errorf("%s: mob #%d: unknown default_position %q", path, md.Vnum, md.DefaultPosition)
	}
	if sex, ok := game.ValueByName(md.Sex, game.NativeSexNames()); ok {
		mob.Sex = sex
	} else {
...
func (l *loader) objFromDoc(path string, od objDoc) *game.ObjDef {
	...
	typ, ok := game.ValueByName(od.Type, game.NativeItemTypeNames())
	if !ok {
		l.errorf("%s: object #%d: unknown type %q", path, od.Vnum, od.Type)
	}
	obj.Type = typ
	obj.WearFlags = l.flags(path, od.Wear, game.NativeWearFlagNames(), "wear flag", od.Vnum).Set(game.Flags(od.WearRaw))
	obj.ExtraFlags = l.flags(path, od.Flags, game.NativeItemExtraFlagNames(), "item flag", od.Vnum).Set(game.Flags(od.FlagsRaw))
	permAffect, unknown := game.ParseBitNames(od.PermAffect, game.NativeAffectFlagNames())
	if len(unknown) > 0 {
	...
	for _, ad := range od.Affects {
		loc, ok := game.ValueByName(ad.Location, game.NativeApplyTypeNames())
		if !ok {
...
func (l *loader) shopFromDoc(path string, sd shopDoc) *game.ShopDef {
	...
	for _, b := range sd.Buys {
		typ, ok := game.ValueByName(b.Type, game.NativeItemTypeNames())
		if !ok {
	...
	shop.Flags = l.flags(path, sd.Flags, game.NativeShopFlagNames(), "shop flag", sd.Vnum)
	trade, unknown := game.ParseBitNames(sd.Refuses, game.NativeShopTradeNames())
	if len(unknown) > 0 {
...
func wearSlotByName(name string) (int32, bool) {
	return game.ValueByName(name, game.NativeWearPositionNames())
}
