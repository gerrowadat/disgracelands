...
// yaml's object-instance schema directly (ObjInstanceDoc et al.) — the
...
// orphaned object file behind forever. yaml's Save replaces the whole
...
// houseSchema is the document's schema tag, docs/design/data-format.md
// §10.1.
const houseSchema = "dl/houses@1"

// fileName is state/houses.yaml under whatever directory Config.ObjectDir
// names — yaml folds hcontrol and the per-room object files into the one
...
type houseDoc struct {
	...
	Contents    []playeryaml.ObjInstanceDoc `yaml:"contents,omitempty"`
}
...
func (s *Store) load() error {
	...
	for _, hd := range d.Houses {
		...
		for _, od := range hd.Contents {
			st, unknown := playeryaml.StoredObjectFromDoc(od)
			if len(unknown) > 0 {
...
func (s *Store) save() error {
	...
	for _, vnum := range vnums {
		...
		for _, obj := range e.contents {
			hd.Contents = append(hd.Contents, playeryaml.ObjInstanceDocFrom(obj))
		}
