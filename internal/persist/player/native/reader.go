...
func recordFromDoc(doc *playerDoc) (*game.PlayerRecord, []string, error) {
	...
	sex, ok := game.ValueByName(doc.Identity.Sex, game.NativeSexNames())
	if doc.Identity.Sex != "" && !ok {
		note("identity.sex: unknown value %q", doc.Identity.Sex)
	}
	class, ok := game.ValueByName(doc.Identity.Class, game.NativeClassNames())
	if doc.Identity.Class != "" && !ok {
		note("identity.class: unknown value %q", doc.Identity.Class)
	}
	remort, remortUnknown := game.ParseBitNames(doc.Identity.Remort, game.NativeClassNames())
	...
	act, actUnknown := game.ParseBitNames(doc.Flags.Act, game.NativePlayerFlagNames())
	for _, name := range actUnknown {
		note("flags.act: unknown name %q", name)
	}
	aff, affUnknown := game.ParseBitNames(doc.Flags.Affected, game.NativeAffectFlagNames())
	for _, name := range affUnknown {
		note("flags.affected: unknown name %q", name)
	}
	prefs, prefsUnknown := game.ParseBitNames(doc.Flags.Prefs, game.NativePreferenceNames())
...
func affectsFromDoc(doc []affectDoc) ([]game.Affect, []string) {
	...
	for _, ad := range doc {
		...
		location, ok := game.ValueByName(ad.Location, game.NativeApplyTypeNames())
		if ad.Location != "" && !ok {
			unknown = append(unknown, fmt.Sprintf("affects: unknown location %q", ad.Location))
		}
		bits, bitsUnknown := game.ParseBitNames(ad.Sets, game.NativeAffectFlagNames())
...
func StoredObjectFromDoc(od ObjInstanceDoc) (player.StoredObject, []string) {
	...
	perm, permUnknown := game.ParseBitNames(od.PermAffect, game.NativeAffectFlagNames())
