...
// are called at different times over the same file (native.go's
// read-merge-write).
func docFromRecord(rec *game.PlayerRecord) playerDoc {
	sex, _ := game.NameByValue(rec.Sex, game.NativeSexNames())
	class, _ := game.NameByValue(rec.Class, game.NativeClassNames())
	remort, remortRaw := game.NameBits(game.Flags(rec.RemortVector), game.NativeClassNames()) //nolint:gosec // a small per-class bitmask
	act, actRaw := game.NameBits(rec.PlayerFlags, game.NativePlayerFlagNames())
	aff, affRaw := game.NameBits(rec.AffectFlags, game.NativeAffectFlagNames())
	prefs, prefsRaw := game.NameBits(rec.Preferences, game.NativePreferenceNames())
...
func affectsDocFrom(affects []game.Affect) []affectDoc {
	...
	for _, a := range affects {
		sets, setsRaw := game.NameBits(a.Bits, game.NativeAffectFlagNames())
...
// read-merge-write native.go's Store does over one file.
